...
テキスト原稿・Markdownともに `|本文《ルビ》` と書くと、本文の上にルビを表示します。
全角の `｜本文《ルビ》` も使えます。原稿ファイルはそのまま保ち、Web公開時に変換します。
...
`《ケイオスディーラー》` のように縦棒のない表記は変換しません。
本文・ルビは同じ行に書き、どちらも空にしないでください。
Markdownではコード内は変換せず、本文・ルビの中に装飾構文を入れずに使います。
表のセル内では、列区切りと混同しない全角の `｜` を使ってください。
Markdownで構文をそのまま見せるときはコード記法、または半角の縦棒を `\|` と書きます。
